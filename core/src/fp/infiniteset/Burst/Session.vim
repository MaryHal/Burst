...
map! <F31> ä
noremap! <M-BS> 
noremap! <expr> <SNR>39_transpose "\<BS>\<Right>".matchstr(getcmdline()[0 : getcmdpos()-2], '.$')
noremap! <expr> <SNR>39_transposition getcmdpos()>strlen(getcmdline())?"\<Left>":getcmdpos()>1?'':"\<Right>"
...
nnoremap <silent>  x :Unite -buffer-name=commands command
nnoremap <silent>  n :Unite -buffer-name=find find:.
nnoremap <silent>  l :UniteWithCursorWord -buffer-name=search_file line
nnoremap <silent>  h :Unite -buffer-name=help help
nnoremap <silent>  g :Unite -buffer-name=grep grep:.
nnoremap <silent>  p :Unite -buffer-name=files file_rec/async
nnoremap <silent>   :UniteWithBufferDir -buffer-name=files file file/new
nnoremap <silent>  F :UniteWithCurrentDir -buffer-name=files file file/new
nnoremap <silent>  f :Unite -buffer-name=files file file/new
nnoremap <silent>   :UniteWithBufferDir -buffer-name=change-cwd -default-action=lcd directory
nnoremap <silent>  D :UniteWithCurrentDir -buffer-name=change-cwd -default-action=lcd directory
nnoremap <silent>  d :Unite -buffer-name=change-cwd -default-action=lcd directory
nnoremap <silent>  a :Unite -buffer-name=sources source
nnoremap <silent>  o :Unite -buffer-name=outline -vertical outline
nnoremap <silent>  y :Unite -buffer-name=yanks history/yank
nnoremap <silent>  u :Unite -buffer-name=buffers buffer
nnoremap <silent>  r :Unite -buffer-name=register register
omap    <Plug>(easymotion-s)
nmap    <Plug>(easymotion-s)
nnoremap  c :cd %:p:h
nnoremap <silent>  t :!$TERMINAL -e fish
xnoremap < <gv
xnoremap > >gv
noremap! ð <Up>
noremap! î <Down>
...
nnoremap <silent> ø :Unite -buffer-name=commands command
let &cpo=s:cpo_save
unlet s:cpo_save
set autoindent
set autoread
...
set redrawtime=100
set ruler
set runtimepath=~/.vim,~/.vim/bundle/ag.vim,~/.vim/bundle/bufkill.vim,~/.vim/bundle/gundo.vim,~/.vim/bundle/neocomplete.vim,~/.vim/bundle/unite-help,~/.vim/bundle/unite-outline,~/.vim/bundle/unite.vim,~/.vim/bundle/vim-commentary,~/.vim/bundle/vim-easymotion,~/.vim/bundle/vim-eunuch,~/.vim/bundle/vim-hemisu,~/.vim/bundle/vim-marching,~/.vim/bundle/vim-markdown,~/.vim/bundle/vim-reunions,~/.vim/bundle/vim-rsi,~/.vim/bundle/vimfiler.vim,~/.vim/bundle/vimproc,/usr/share/vim/vimfiles,/usr/share/vim/vim74,/usr/share/vim/vimfiles/after,~/.vim/bundle/vim-marching/after,~/.vim/after,/usr/share/vim/vimfiles/eclim,/usr/share/vim/vimfiles/eclim/after
set scrolloff=10
set shiftwidth=4
set shortmess=Iat
set showbreak=↪
...
set splitbelow
set splitright
set statusline=%{winnr('$')>1?'['.winnr().'/'.winnr('$').(winnr('#')==winnr()?'#':'').']':''}\ %{(&previewwindow?'[preview]\ ':'').expand('%:t:.')}\ %=%m%y%{'['.(&fenc!=''?&fenc:&enc).','.&ff.']'}%{printf('\ \ %4d/%d',line('.'),line('$'))}
set suffixes=.aux,.log,.dvi,.bbl,.blg,.brf,.cb,.ind,.idx,.ilg,.inx,.out,.toc,.class
set noswapfile
...
set wildignore=*.o,*.pyc,*.hi,.hg,.git,.svn,.gitignore,*.aux,*.out,*.toc,*.jpg,*.bmp,*.gif,*.png,*.jpeg,*.o,*.obj,*.dll,*.manifest,*.d,*.spl,*.sw?,*.DS_Store,*.luac,*.wav,*.mp3,*.ogg,*.png,*.jpg,*.gif,*.so,*.swp,*.zip,*.pdf,*.exe,*.jar,*.class
set wildignorecase
set wildmenu
...
silent only
cd ~/code/current/Burst/core/src/fp/infiniteset/Burst
if expand('%') == '' && !&modified && line('$') <= 1 && getline(1) == ''
  let s:wipebuf = bufnr('%')
endif
set shortmess=aoO
badd +21 GameController.java
badd +0 SimpleController.java
badd +0 Screens/SimpleScreen.java
badd +0 Beats/BeatMap.java
badd +1 Fireworks/Firework.java
badd +9 Fireworks/FireworkGroup.java
badd +0 Fireworks/FireworkLauncher.java
argglobal
silent! argdel *
set stal=2
edit GameController.java
...
11
normal! 0
lcd ~/code/current/Burst/core/src/fp/infiniteset/Burst
wincmd w
argglobal
edit ~/code/current/Burst/core/src/fp/infiniteset/Burst/SimpleController.java
inoreabbr <buffer> logger logger=eclim#java#logging#LoggingInit("logger")
inoreabbr <buffer> log log=eclim#java#logging#LoggingInit("log")
setlocal keymap=
setlocal noarabic
...
setlocal wrapmargin=0
silent! normal! zE
let s:l = 79 - ((15 * winheight(0) + 25) / 51)
if s:l < 1 | let s:l = 1 | endif
exe s:l
normal! zt
79
...
exe 'vert 1resize ' . ((&columns * 96 + 96) / 193)
exe 'vert 2resize ' . ((&columns * 96 + 96) / 193)
tabedit ~/code/current/Burst/core/src/fp/infiniteset/Burst/Screens/SimpleScreen.java
set splitbelow splitright
wincmd t
set winheight=1 winwidth=1
argglobal
...
lcd ~/code/current/Burst/core/src/fp/infiniteset/Burst
2wincmd w
tabedit ~/code/current/Burst/core/src/fp/infiniteset/Burst/Beats/BeatMap.java
set splitbelow splitright
wincmd t
set winheight=1 winwidth=1
argglobal
...
setlocal wrapmargin=0
silent! normal! zE
let s:l = 160 - ((27 * winheight(0) + 25) / 51)
if s:l < 1 | let s:l = 1 | endif
exe s:l
normal! zt
160
...
lcd ~/code/current/Burst/core/src/fp/infiniteset/Burst
2wincmd w
tabedit ~/code/current/Burst/core/src/fp/infiniteset/Burst/Fireworks/Firework.java
set splitbelow splitright
wincmd _ | wincmd |
vsplit
1wincmd h
...
wincmd w
argglobal
edit ~/code/current/Burst/core/src/fp/infiniteset/Burst/Fireworks/FireworkGroup.java
inoreabbr <buffer> logger logger=eclim#java#logging#LoggingInit("logger")
inoreabbr <buffer> log log=eclim#java#logging#LoggingInit("log")
setlocal keymap=
setlocal noarabic
...
wincmd w
argglobal
edit ~/code/current/Burst/core/src/fp/infiniteset/Burst/Fireworks/FireworkLauncher.java
inoreabbr <buffer> logger logger=eclim#java#logging#LoggingInit("logger")
inoreabbr <buffer> log log=eclim#java#logging#LoggingInit("log")
setlocal keymap=
setlocal noarabic
...
setlocal wrapmargin=0
silent! normal! zE
let s:l = 125 - ((14 * winheight(0) + 12) / 25)
if s:l < 1 | let s:l = 1 | endif
exe s:l
normal! zt
125
...
unlet! s:wipebuf
set winheight=1 winwidth=20 shortmess=Iat
